...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('1. Indicador de carregamento mostrado.');

    // Coletar dados do formulário
    const formData = new FormData(rentalForm);
    console.log('2. Dados do formulário coletados.');
    ...
    try {
        ...
        console.log('4. Resposta do servidor recebida.');

        const result = await response.json();
        console.log('5. Resposta do servidor processada como JSON. Conteúdo:', result);

        if (result.success) {
            console.log('6. Sucesso no backend. Redirecionando para a página de sucesso.');
            window.location.href = 'sucesso.html';
        } else {
            console.error('7. Erro no backend. Mensagem:', result.message);
            window.location.href = 'erro.html';
        }
    } catch (error) {
        console.error('8. Erro de rede ou no servidor:', error);
        window.location.href = 'erro.html';
    } finally {
        // Esconder o indicador e habilitar o botão novamente
        loadingIndicator.style.display = 'none';
        submitBtn.disabled = false;
        console.log('9. Processo finalizado. Indicador de carregamento escondido.');
    }
});
});